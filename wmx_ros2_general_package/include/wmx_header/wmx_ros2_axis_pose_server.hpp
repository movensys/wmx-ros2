...
class WmxRos2AxisPoseServer : public rclcpp::Node
{
  public:
    ...
    explicit AxisPoseServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions()) : Node("wmx_ros2_axis_pose_server", options)
    {
      ...
      this->wmx_ros2_axis_pose_server_ = rclcpp_action::create_server<wmx_ros2_message::action::AxisPose>(
  ...
  private:
    rclcpp_action::Server<wmx_ros2_message::action::AxisPose>::SharedPtr wmx_ros2_axis_pose_server_;

    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const wmx_ros2_message::action::AxisPose::Goal> goal)
    {
      RCLCPP_INFO(this->get_logger(), "Get axis target pose");
      (void)uuid;
