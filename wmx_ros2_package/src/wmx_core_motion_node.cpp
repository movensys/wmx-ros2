...
WmxCoreMotionNode::WmxCoreMotionNode() : Node("wmx_core_motion_node")
{
  ...
  engineReadySub_ = this->create_subscription<std_msgs::msg::Bool>(
      "wmx/engine/ready", ready_qos, std::bind(&WmxCoreMotionNode::onEngineReady, this, _1),
      sub_opts);

  coreMotionReadyPub_ =
      this->create_publisher<std_msgs::msg::Bool>("wmx/core_motion/ready", ready_qos);

  setAxisOnService_ = this->create_service<wmx_ros2_message::srv::SetAxis>(
      "wmx/axis/set_on", std::bind(&WmxCoreMotionNode::setAxisOn, this, _1, _2));

  clearAlarmService_ = this->create_service<wmx_ros2_message::srv::SetAxis>(
      "wmx/axis/clear_alarm", std::bind(&WmxCoreMotionNode::clearAlarm, this, _1, _2));

  setAxisModeService_ = this->create_service<wmx_ros2_message::srv::SetAxis>(
      "wmx/axis/set_mode", std::bind(&WmxCoreMotionNode::setAxisMode, this, _1, _2));

  setAxisPolarityService_ = this->create_service<wmx_ros2_message::srv::SetAxis>(
      "wmx/axis/set_polarity", std::bind(&WmxCoreMotionNode::setAxisPolarity, this, _1, _2));

  setAxisGearRatioService_ = this->create_service<wmx_ros2_message::srv::SetAxisGearRatio>(
      "wmx/axis/set_gear_ratio", std::bind(&WmxCoreMotionNode::setAxisGearRatio, this, _1, _2));

  setHomingService_ = this->create_service<wmx_ros2_message::srv::SetAxis>(
      "wmx/axis/homing", std::bind(&WmxCoreMotionNode::setHoming, this, _1, _2));

  loadParamsService_ = this->create_service<wmx_ros2_message::srv::LoadWmxParams>(
      "wmx/params/load", std::bind(&WmxCoreMotionNode::loadWmxParams, this, _1, _2));

  getParamsService_ = this->create_service<wmx_ros2_message::srv::GetWmxParams>(
      "wmx/params/get", std::bind(&WmxCoreMotionNode::getWmxParams, this, _1, _2));
...
void WmxCoreMotionNode::onEngineReady(const std_msgs::msg::Bool::SharedPtr msg)
{
  ...
  axisStatePub_ = this->create_publisher<wmx_ros2_message::msg::AxisState>("wmx/axis/state", 1);

  axisVelSub_ = this->create_subscription<wmx_ros2_message::msg::AxisVelocity>(
      "wmx/axis/velocity", 1, std::bind(&WmxCoreMotionNode::axisVelCallback, this, _1));

  axisPoseSub_ = this->create_subscription<wmx_ros2_message::msg::AxisPose>(
      "wmx/axis/position", 1, std::bind(&WmxCoreMotionNode::axisPoseCallback, this, _1));
  ...
  axisStateTimer_ = this->create_wall_timer(std::chrono::milliseconds(1000 / rate_),
...
void WmxCoreMotionNode::axisPoseCallback(const wmx_ros2_message::msg::AxisPose::SharedPtr msg)
{
  ...
  for (size_t i = 0; i < axis_count; i++) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      RCLCPP_ERROR(this->get_logger(), "Failed to move position motor %d. Error=%d (%s)",
...
void WmxCoreMotionNode::axisPoseRelativeCallback(
    const wmx_ros2_message::msg::AxisPose::SharedPtr msg)
{
  size_t axis_count = msg->index.size();
  for (size_t i = 0; i < axis_count; i++) {
    position_.axis = msg->index[i];
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      RCLCPP_ERROR(this->get_logger(), "Failed to move relative motor %d. Error=%d (%s)",
                   msg->index[i], err_, errString_);
    }
  }
}
...
void WmxCoreMotionNode::axisVelCallback(const wmx_ros2_message::msg::AxisVelocity::SharedPtr msg)
{
  ...
  for (size_t i = 0; i < axis_count; i++) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      RCLCPP_ERROR(this->get_logger(), "Failed to move velocity motor %d. Error=%d (%s)",
                   msg->index[i], err_, errString_);
    }
  }
}
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxis::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      snprintf(buffer_, sizeof(buffer_), "Failed to set axis %d %s. Error=%d (%s)", axis_index,
               on_off ? "on" : "off", err_, errString_);
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxis::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      snprintf(buffer_, sizeof(buffer_), "Failed to set axis %d mode %d. Error=%d (%s)", axis_index,
               mode_value, err_, errString_);
      RCLCPP_ERROR(this->get_logger(), "%s", buffer_);
      all_success = false;
    } else {
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxis::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      snprintf(buffer_, sizeof(buffer_), "Failed to clear alarm axis %d. Error=%d (%s)",
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxis::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    if (request->data[i] != 1 && request->data[i] != -1) {
      snprintf(buffer_, sizeof(buffer_), "Invalid polarity value for axis %d: %d",
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxisGearRatio::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    err_ = wmx3LibCm_->config->SetGearRatio(request->index[i], request->numerator[i],
                                            request->denominator[i]);
    if (err_ != ErrorCode::None) {
      all_success = false;
    ...
    std::shared_ptr<wmx_ros2_message::srv::SetAxis::Response> response)
{
  ...
  for (size_t i = 0; i < request->index.size(); ++i) {
    ...
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
      snprintf(buffer_, sizeof(buffer_), "Failed to home axis %d. Error=%d (%s)", request->index[i],
               err_, errString_);
      RCLCPP_ERROR(this->get_logger(), "%s", buffer_);
      all_success = false;
    } else {
    ...
    std::shared_ptr<wmx_ros2_message::srv::LoadWmxParams::Response> response)
{
  ...
  err_ = wmx3LibCm_->config->ImportAndSetAll(const_cast<char *>(request->file_path.c_str()),
                                             &sysParamErr, &axisParamErr);
    ...
    std::shared_ptr<wmx_ros2_message::srv::GetWmxParams::Response> response)
{
  ...
  for (int32_t i : request->index) {
    ...
    lines.push_back("  AxisPolarity       = " +
                    std::to_string(static_cast<int>(axisParam.axisPolarity[i])));
    lines.push_back("  CommandMode        = " +
