...
void GripperController::setRosParameter(){
  ...
  this->declare_parameter<int>("bytes_address", 0);
  this->declare_parameter<int>("bit_address", 0);

  this->get_parameter("wmx_gripper_topic", wmxGripperTopic_);
  this->get_parameter("bytes_address", bytesAddress_);
  this->get_parameter("bit_address", bitAddress_);
  

  // Print parameter values
  RCLCPP_INFO(this->get_logger(), "===== ROS2 Parameters =====");
  RCLCPP_INFO(this->get_logger(), "wmx_gripper_topic: %s", wmxGripperTopic_.c_str());
  RCLCPP_INFO(this->get_logger(), "bytes_address: %d", bytesAddress_);
  RCLCPP_INFO(this->get_logger(), "bit_address: %d", bitAddress_);
...
void GripperController::setGripper(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                                        std::shared_ptr<std_srvs::srv::SetBool::Response> response){
  if (request->data) {
    err_ = Wmx3Lib_Io_.SetOutBit(bytesAddress_, bitAddress_, 1);
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
    ...
    err_ = Wmx3Lib_Io_.SetOutBit(bytesAddress_, bitAddress_, 0);
    if (err_ != ErrorCode::None) {
      wmx3Lib_.ErrorToString(err_, errString_, sizeof(errString_));
